import { Controller, Post, HttpCode, HttpStatus, UseGuards, Body, UseInterceptors, UploadedFile, BadRequestException } from '@nestjs/common';
...
import { UserRole, NewsIngestType, ModerationStatus, ManualInputType } from '@prisma/client';
...
export class AdminNewsController {
    ...
    async createManualNews(
        ...
        if (dto.inputType === ManualInputType.FILE) {
            if (!file) throw new BadRequestException('File is required for FILE input type');
            ...
            if (!dto.linkUrl) throw new BadRequestException('Link URL is required for LINK input type');
            ...
            if (!dto.textContent) throw new BadRequestException('Text content is required for TEXT input type');
