...
export class AuthService {
    ...
    private readonly SESSION_DURATION_DAYS = parseInt(process.env.SESSION_DURATION_DAYS || '9');
     ...
     */
    async login(dto: LoginDto, deviceInfo?: string, ipAddress?: string): Promise<{ user: any; sessionToken: string }> {
        // Find user
        ...
        if (user.isTrial && (user as any).subscription) {
            const now = new Date();
            if ((user as any).subscription.endsAt && (user as any).subscription.endsAt < now) {
